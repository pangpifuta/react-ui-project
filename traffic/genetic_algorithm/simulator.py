...
# Method 2: locally run modified version of TSF
    def requestStats2(self, i):
        ...
        if self.savingState:
            ...
            request.append(" saved_state_" +
                           str((self.previousSave+1) % 2)+".txt")
        elif self.useSave:
            request.append("saved_state_"+str(self.previousSave)+".txt")
            request.append(" end_"+str((self.timeStepNum+1) %
                                       2)+"_"+str(i)+".txt")
        else:
...
# Method 1: Use TSF microservice
    ...
    def requestMany(self, number):
        ...
        Parallel(n_jobs=N_JOBS, require='sharedmem')(
            delayed(self.requestStats2)(i) for i in range(number))
